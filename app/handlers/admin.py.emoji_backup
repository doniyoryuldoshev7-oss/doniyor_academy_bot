...
def admin_menu():
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="РІС›вЂў Fan qoРІР‚Вshish", callback_data="adm:add_subject"
                )
            ],
            [
                InlineKeyboardButton(
                    text="РІС›вЂў Mavzu qoРІР‚Вshish", callback_data="adm:add_topic"
                )
            ],
            [
                InlineKeyboardButton(
                    text="РІС›вЂў Savol qoРІР‚Вshish", callback_data="adm:add_question"
                )
            ],
            [
                InlineKeyboardButton(
                    text="СЂСџвЂњТђ Excel/CSV import", callback_data="adm:import"
                )
            ],
            [
                InlineKeyboardButton(
                    text="СЂСџвЂњС™ Fanlar / mavzular", callback_data="adm:catalog"
                )
            ],
            [
                InlineKeyboardButton(
            ...
            [InlineKeyboardButton(text="СЂСџвЂњР‰ Statistika", callback_data="adm:stats")],
            [InlineKeyboardButton(text="СЂСџвЂњСћ E'lonlar", callback_data="adm:send_ad")],
            [InlineKeyboardButton(text="РІСњРЉ Bekor qilish", callback_data="adm:cancel")],
        ]
    )
...
def subject_kb(subjects, prefix="adm:qsub"):
    rows = [
        [InlineKeyboardButton(text=f"СЂСџвЂњС™ {s.name}", callback_data=f"{prefix}:{s.id}")]
        for s in subjects
    ]
    rows.append(
        [InlineKeyboardButton(text="РІСњРЉ Bekor qilish", callback_data="adm:cancel")]
...
def topic_kb(topics, prefix="adm:qtopic"):
    rows = [
        [InlineKeyboardButton(text=f"СЂСџвЂњвЂ“ {t.name}", callback_data=f"{prefix}:{t.id}")]
        for t in topics
    ]
    rows.append(
        [InlineKeyboardButton(text="РІВ¬вЂ¦РїС‘РЏ Fan tanlash", callback_data="adm:add_question")]
    )
    rows.append(
        [InlineKeyboardButton(text="РІСњРЉ Bekor qilish", callback_data="adm:cancel")]
    )
...
async def show_admin(target, answer=True):
    text = "РІС™в„ўРїС‘РЏ <b>Doniyor Academy РІР‚вЂќ Admin panel</b>\n\n" "Kerakli amalni tanlang:"
...
async def cb_admin(c: CallbackQuery, state: FSMContext):
    if not is_admin(c.from_user.id):
        await c.answer("Sizda admin huquqi yoРІР‚Вq.", show_alert=True)
        return
    await state.clear()
    await show_admin(c)
...
async def cancel(c: CallbackQuery, state: FSMContext):
    ...
    admin_btn = [InlineKeyboardButton(text="РІС™в„ўРїС‘РЏ Admin panel", callback_data="admin")]
    ...
    await c.message.edit_text(
        "РЎР‚РЎСџР вЂ№РІР‚Сљ <b>Doniyor Academy</b>\n\nBosh menyu:",
...
async def add_subject_start(c: CallbackQuery, state: FSMContext):
    ...
    await c.message.edit_text(
        "РІС›вЂў <b>Yangi fan</b>\n\nFan nomini yuboring:\n\nMasalan: <i>OРІР‚Вzbekiston tarixi</i>",
...
async def add_subject_finish(m: Message, state: FSMContext):
    ...
    if not name:
        await m.answer("РІСњвЂ” Fan nomi boРІР‚Вsh boРІР‚Вlishi mumkin emas.")
        return
    async with SessionLocal() as s:
        ...
        if exists:
            await m.answer("РІС™В РїС‘РЏ Bu fan allaqachon mavjud.")
            return
        s.add(Subject(name=name))
        await s.commit()
    await state.clear()
    await m.answer(f"РІСљвЂ¦ Fan qoРІР‚Вshildi: <b>{name}</b>", reply_markup=admin_menu())
...
async def add_topic_start(c: CallbackQuery, state: FSMContext):
    ...
    if not subjects:
        await c.answer("Avval fan qoРІР‚Вshing.", show_alert=True)
        return
    await state.set_state(AdminState.add_topic_subject)
    await c.message.edit_text(
        "СЂСџвЂњС™ <b>Mavzu qoРІР‚Вshish</b>\n\nFanni tanlang:",
...
async def add_topic_subject(c: CallbackQuery, state: FSMContext):
    ...
    await c.message.edit_text("СЂСџвЂњвЂ“ Mavzu nomini yuboring:", reply_markup=cancel_kb())
    await c.answer()
...
async def add_topic_finish(m: Message, state: FSMContext):
    ...
    async with SessionLocal() as s:
        ...
        if exists:
            await m.answer("РІС™В РїС‘РЏ Bu mavzu allaqachon mavjud.")
            return
        s.add(Topic(subject_id=subject.id, name=name))
        await s.commit()
    await state.clear()
    await m.answer(
        f"РІСљвЂ¦ <b>{subject.name}</b> faniga <b>{name}</b> mavzusi qoРІР‚Вshildi.",
...
async def add_question_start(c: CallbackQuery, state: FSMContext):
    ...
    await c.message.edit_text(
        "СЂСџвЂњвЂ№ <b>Yangi test savoli</b>\n\nFanni tanlang:",
...
async def add_question_subject(c: CallbackQuery, state: FSMContext):
    ...
    if not topics:
        await c.answer(
            "Bu fanda hali mavzu yoРІР‚Вq. Avval mavzu qoРІР‚Вshing.", show_alert=True
        )
        return
    await state.update_data(subject_id=sid)
    await state.set_state(AdminState.add_question_topic)
    await c.message.edit_text(
        f"СЂСџвЂњС™ <b>{subject.name}</b>\n\nMavzuni tanlang:", reply_markup=topic_kb(topics)
    )
    await c.answer()
...
async def add_question_topic(c: CallbackQuery, state: FSMContext):
    ...
    await c.message.edit_text(
        f"СЂСџвЂњвЂ“ <b>{topic.name}</b>\n\n1РїС‘РЏРІС“Р€ Savol matnini yuboring:",
        reply_markup=cancel_kb(),
    )
    await c.answer()
...
async def q_text(m: Message, state: FSMContext):
    ...
    if not text:
        await m.answer("РІСњвЂ” Savol matni boРІР‚Вsh boРІР‚Вlishi mumkin emas.")
        return
    await state.update_data(text=text)
    await state.set_state(AdminState.add_question_a)
    await m.answer("2РїС‘РЏРІС“Р€ <b>A variant</b>ni yuboring:", reply_markup=cancel_kb())
...
async def save_option(m: Message, state: FSMContext, key: str, next_state, label: str):
    ...
    if not value:
        await m.answer("РІСњвЂ” Variant boРІР‚Вsh boРІР‚Вlishi mumkin emas.")
        return
...
async def q_a(m, state):
    await save_option(
        ...
        "3РїС‘РЏРІС“Р€ <b>B variant</b>ni yuboring:",
    )
...
async def q_b(m, state):
    await save_option(
        ...
        "4РїС‘РЏРІС“Р€ <b>C variant</b>ni yuboring:",
    )
...
async def q_c(m, state):
    await save_option(
        ...
        "5РїС‘РЏРІС“Р€ <b>D variant</b>ni yuboring:",
    )
...
async def q_d(m, state):
    await save_option(
        ...
        "6РїС‘РЏРІС“Р€ ToРІР‚ВgРІР‚Вri javob harfini yuboring: <b>A</b>, <b>B</b>, <b>C</b> yoki <b>D</b>",
    )
...
async def q_correct(m: Message, state: FSMContext):
    ...
    if correct not in {"A", "B", "C", "D"}:
        await m.answer("РІСњвЂ” Faqat A, B, C yoki D yuboring.")
        return
    await state.update_data(correct_option=correct)
    await state.set_state(AdminState.add_question_explanation)
    await m.answer(
        "7РїС‘РЏРІС“Р€ <b>Izoh</b>ni yuboring. Agar izoh kerak boРІР‚Вlmasa, <code>-</code> yuboring:",
        reply_markup=cancel_kb(),
    )
...
async def q_explanation(m: Message, state: FSMContext):
    ...
    await state.clear()
    await m.answer(
        f"РІСљвЂ¦ <b>Savol muvaffaqiyatli qoРІР‚Вshildi!</b>\n\n"
        f"СЂСџвЂ вЂќ ID: <code>{qid}</code>\n"
        f"СЂСџвЂњвЂ“ Mavzu: <b>{topic.name}</b>\n"
        f"РІСљвЂ¦ ToРІР‚ВgРІР‚Вri javob: <b>{data['correct_option']}</b>",
        reply_markup=admin_menu(),
    )
...
async def import_start(c: CallbackQuery, state: FSMContext):
    ...
    await c.message.edit_text(
        "СЂСџвЂњТђ <b>Excel/CSV orqali test import</b>\n\n"
        "Faylni yuboring: <b>.xlsx</b> yoki <b>.csv</b>\n\n"
        "Ustunlar:\n"
        "<code>Fan | Mavzu | Savol | A | B | C | D | ToРІР‚ВgРІР‚Вri javob | Izoh</code>\n\n"
        "РІР‚Сћ ToРІР‚ВgРІР‚Вri javob: A, B, C yoki D\n"
        "РІР‚Сћ Izoh ixtiyoriy\n"
        "РІР‚Сћ Fan va mavzu mavjud boРІР‚Вlmasa, avtomatik yaratiladi",
        reply_markup=cancel_kb(),
    )
    await c.answer()
...
async def import_document(m: Message, state: FSMContext):
    ...
    if not (filename.endswith(".xlsx") or filename.endswith(".csv")):
        await m.answer(
            "РІСњвЂ” Faqat .xlsx yoki .csv fayl yuboring.", reply_markup=cancel_kb()
        )
        return
    ...
    try:
        ...
        if filename.endswith(".xlsx"):
            ...
            for enc in ("utf-8-sig", "utf-8", "cp1251"):
                try:
                    ...
                    text = None
            if text is None:
                raise ValueError("CSV kodlashini oРІР‚Вqib boРІР‚Вlmadi.")
        ...
        for row_no, row in enumerate(rows[1:], start=2):
            ...
            if not all(vals):
                errors.append(f"{row_no}-qator: majburiy maydon boРІР‚Вsh.")
                continue
            if correct not in {"A", "B", "C", "D"}:
                errors.append(f"{row_no}-qator: toРІР‚ВgРІР‚Вri javob A/B/C/D boРІР‚Вlishi kerak.")
                continue
        ...
        if errors:
            ...
            await m.answer(
                f"РІС™В РїС‘РЏ <b>Faylda xato bor.</b>\n\n{preview}{more}\n\n"
        ...
        await state.clear()
        await m.answer(
            "РЎР‚РЎСџР вЂ№РІР‚В° <b>IMPORT MUVAFFAQIYATLI!</b>\n\n"
            f"СЂСџвЂњвЂ№ QoРІР‚Вshilgan savollar: <b>{len(valid)}</b>\n"
            "СЂСџвЂњС™ Fan va mavzular avtomatik bogРІР‚Вlandi.",
            reply_markup=admin_menu(),
        )
    except Exception as e:
        await m.answer(
...
async def import_wrong_type(m: Message, state: FSMContext):
    if not is_admin(m.from_user.id):
        return
    await m.answer(
        "РІСњвЂ” Excel/CSV faylini hujjat sifatida yuboring: <b>.xlsx</b> yoki <b>.csv</b>.",
        reply_markup=cancel_kb(),
    )
...
async def catalog(c: CallbackQuery):
    ...
    if not subjects:
        await c.answer("Hali fanlar yoРІР‚Вq.", show_alert=True)
        return

    buttons = [
        [
            InlineKeyboardButton(
                text=f"СЂСџвЂњС™ {sub.name}", callback_data=f"adm:subject:{sub.id}"
            )
        ]
        for sub in subjects
    ]

    buttons.append([InlineKeyboardButton(text="РІВ¬вЂ¦РїС‘РЏ Admin panel", callback_data="admin")])
...
async def catalog_subject(c: CallbackQuery):
    ...
    buttons = [
        [
            InlineKeyboardButton(
                text=f"СЂСџвЂњвЂ“ {topic.name}", callback_data=f"adm:topic:{topic.id}"
            )
        ]
        for topic in topics
    ]
    ...
    buttons.append(
        [
            InlineKeyboardButton(
                text="СЂСџвЂ”вЂ Fanni oРІР‚Вchirish",
    ...
    buttons.append(
        [InlineKeyboardButton(text="РІВ¬вЂ¦РїС‘РЏ Fanlar", callback_data="adm:catalog")]
    )

    await c.message.edit_text(
        f"СЂСџвЂњС™ <b>{subject.name}</b>\n\n"
        f"СЂСџвЂњвЂ“ Mavzular: <b>{topic_count} ta</b>\n\n"
        "Kerakli amalni tanlang:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons),
        parse_mode="HTML",
    )
...
async def catalog_topic(c: CallbackQuery):
    ...
    buttons = [
        [
            InlineKeyboardButton(
                text="РЎР‚РЎСџРІР‚СљРІР‚в„– Savollarni koРІР‚Вrish", callback_data=f"adm:questions:{tid}"
            )
        ],
        [
            InlineKeyboardButton(
                text="РІСљРЏРїС‘РЏ Mavzuni tahrirlash", callback_data=f"adm:edit_topic:{tid}"
            )
        ],
        [
            InlineKeyboardButton(
                text="СЂСџвЂ”вЂ Mavzuni oРІР‚Вchirish", callback_data=f"adm:delete_topic:{tid}"
            )
        ],
        [
            InlineKeyboardButton(
                text="РІВ¬вЂ¦РїС‘РЏ Mavzular", callback_data=f"adm:subject:{topic.subject_id}"
            )
        ],
    ]

    await c.message.edit_text(
        f"СЂСџвЂњвЂ“ <b>{topic.name}</b>\n\n" f"СЂСџвЂњвЂ№ Savollar: <b>{count}</b>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons),
    )
    await c.answer()
...
async def edit_topic_start(c: CallbackQuery, state: FSMContext):
    ...
    await c.message.edit_text(
        "РІСљРЏРїС‘РЏ <b>MAVZUNI TAHRIRLASH</b>\n\n"
...
async def edit_topic_finish(m: Message, state: FSMContext):
    ...
    if not name:
        await m.answer("РІСњвЂ” Mavzu nomi boРІР‚Вsh boРІР‚Вlishi mumkin emas.")
        return
    ...
    await m.answer(
        "РІСљвЂ¦ <b>MAVZU MUVAFFAQIYATLI TAHRIRLANDI!</b>\n\n" f"Yangi nom: <b>{name}</b>",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="СЂСџвЂњвЂ“ Mavzuni koРІР‚Вrish", callback_data=f"adm:topic:{tid}"
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="РІВ¬вЂ¦РїС‘РЏ Mavzular", callback_data=f"adm:subject:{subject_id}"
                    )
                ],
...
async def delete_topic(c: CallbackQuery):
    ...
    await c.message.edit_text(
        "СЂСџвЂ”вЂ <b>MAVZU OРІР‚ВCHIRILDI!</b>",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
    ...
    await c.answer("Mavzu oРІР‚Вchirildi.")
...
async def catalog_questions(c: CallbackQuery):
    ...
    if not questions:
        await c.message.edit_text(
            f"СЂСџвЂњвЂ“ <b>{topic.name}</b>\n\n" "СЂСџвЂњвЂ№ Bu mavzuda hozircha savollar yoРІР‚Вq.",
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="РІВ¬вЂ¦РїС‘РЏ Mavzu", callback_data=f"adm:topic:{tid}"
                        )
                    ]
                ]
            ),
    ...
    lines = [f"СЂСџвЂњвЂ“ <b>{topic.name}</b>", f"СЂСџвЂњвЂ№ Savollar: <b>{len(questions)}</b>", ""]
    ...
    buttons.append(
        [InlineKeyboardButton(text="РІВ¬вЂ¦РїС‘РЏ Mavzu", callback_data=f"adm:topic:{tid}")]
    )
...
async def catalog_question(c: CallbackQuery):
    ...
    text = (
        "СЂСџвЂњвЂ№ <b>SAVOL</b>\n"
        "РІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓ\n\n"
        f"РІСњвЂњ <b>{q.text}</b>\n\n"
        f"A) {q.option_a}\n"
        f"B) {q.option_b}\n"
        f"C) {q.option_c}\n"
        f"D) {q.option_d}\n\n"
        f"РІСљвЂ¦ ToРІР‚ВgРІР‚Вri javob: <b>{q.correct_option}</b>"
    )

    if q.explanation:
        text += f"\n\nСЂСџвЂ™РЋ <b>Izoh:</b> {q.explanation}"
    ...
    await c.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="РІСљРЏРїС‘РЏ Tahrirlash", callback_data=f"adm:edit_question:{q.id}"
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="СЂСџвЂ”вЂ OРІР‚Вchirish", callback_data=f"adm:delete_question:{q.id}"
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="РІВ¬вЂ¦РїС‘РЏ Savollar", callback_data=f"adm:questions:{q.topic_id}"
                    )
                ],
                [InlineKeyboardButton(text="СЂСџРЏВ Р’В  Admin panel", callback_data="admin")],
            ]
...
async def edit_question_start(c: CallbackQuery, state: FSMContext):
    ...
    await c.message.edit_text(
        "РІСљРЏРїС‘РЏ <b>SAVOLNI TAHRIRLASH</b>\n\n"
        f"Eski savol:\n<b>{q.text}</b>\n\n"
...
async def edit_question_a_handler(m: Message, state: FSMContext):
    ...
    await m.answer("3РїС‘РЏРІС“Р€ <b>B variant</b>ni yuboring:", reply_markup=cancel_kb())
...
async def edit_question_b_handler(m: Message, state: FSMContext):
    ...
    await m.answer("4РїС‘РЏРІС“Р€ <b>C variant</b>ni yuboring:", reply_markup=cancel_kb())
...
async def edit_question_c_handler(m: Message, state: FSMContext):
    ...
    await m.answer("5РїС‘РЏРІС“Р€ <b>D variant</b>ni yuboring:", reply_markup=cancel_kb())
...
async def edit_question_d_handler(m: Message, state: FSMContext):
    ...
    await m.answer(
        "РІСљвЂ¦ <b>SAVOL MUVAFFAQIYATLI TAHRIRLANDI!</b>\n\n"
        "Savol va barcha variantlar yangilandi.",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="РЎР‚РЎСџРІР‚СљРІР‚в„– Savolni koРІР‚Вrish", callback_data=f"adm:question:{qid}"
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="РІВ¬вЂ¦РїС‘РЏ Savollar", callback_data=f"adm:questions:{topic_id}"
                    )
                ],
                [InlineKeyboardButton(text="СЂСџРЏВ Р’В  Admin panel", callback_data="admin")],
            ]
...
async def delete_question(c: CallbackQuery):
    ...
    await c.message.edit_text(
        "СЂСџвЂ”вЂ <b>SAVOL OРІР‚ВCHIRILDI!</b>",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
    ...
    await c.answer("Savol oРІР‚Вchirildi.")
...
async def admin_statistics(c: CallbackQuery):
    ...
    async with SessionLocal() as s:
        ...
        if top_user_res:
            top_student_text = f"СЂСџРЏвЂ  <b>{top_user_res[0]}</b> ({top_user_res[1]} ball)"
        else:
            top_student_text = "Hali mavjud emas"

    # СЂСџвЂњвЂ№ STATISTIKA MATNINI SHAKLLANTIRISH
    stats_text = (
        "СЂСџвЂњР‰ <b>Doniyor Academy РІР‚вЂќ Tizim statistikasi</b>\n"
        "РІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓ\n\n"
        "СЂСџвЂТђ <b>Foydalanuvchilar faolligi:</b>\n"
        f"РІР‚Сћ Jami oРІР‚Вquvchilar: <b>{total_users} ta</b>\n"
        f"РІР‚Сћ Bugun qoРІР‚Вshilganlar: <b>+{new_users_today} ta</b>\n"
        f"РІР‚Сћ Haftalik faol (aktiv): <b>{active_users_7d} ta</b>\n\n"
        "СЂСџвЂњвЂ№ <b>Testlar sifati tahlili:</b>\n"
        f"РІР‚Сћ Jami yakunlangan testlar: <b>{total_attempts} marta</b>\n"
        f"РІР‚Сћ ToРІР‚ВgРІР‚Вri javoblar foizi: <b>{success_pct:.1f}%</b>\n"
        f"РІР‚Сћ РІСљвЂ¦ ToРІР‚ВgРІР‚Вri: {correct_answers} ta | РІСњРЉ Xato: {wrong_answers} ta\n\n"
        "СЂСџвЂњСћ <b>E'lonlar va kontent samaradorligi:</b>\n"
        f"РІР‚Сћ Mavzularni koРІР‚Вrishlar soni: <b>{total_views} marta</b>\n\n"
        "СЂСџРЏвЂ  <b>Reyting va ragРІР‚Вbatlantirish:</b>\n"
        f"РІР‚Сћ Eng yuqori balli talaba: {top_student_text}\n"
        "РІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓРІвЂќРѓ"
    )
...
async def addsubject_command(m: Message):
    ...
    async with SessionLocal() as s:
        ...
        s.add(Subject(name=name))
        await s.commit()
    await m.answer(f"РІСљвЂ¦ Fan qoРІР‚Вshildi: <b>{name}</b>")
...
async def addtopic_command(m: Message):
    ...
    async with SessionLocal() as s:
        ...
        await s.commit()
    await m.answer(f"РІСљвЂ¦ Mavzu qoРІР‚Вshildi: <b>{tn}</b>")
...
async def users(m: Message):
    ...
    await m.answer(f"СЂСџвЂТђ Foydalanuvchilar: <b>{n}</b>")
...
async def broadcast(m: Message):
    ...
    for u in us:
        try:
            ...
            pass
    await m.answer(f"СЂСџвЂњСћ Yuborildi: {sent}/{len(us)}")
...
# 1. Admin panelda "СЂСџвЂњСћ E'lonlar" tugmasi bosilganda ishlaydi
...
async def start_ad(c: CallbackQuery, state: FSMContext):
    ...
    await c.message.answer(
        "СЂСџвЂњСћ E'lon bo'limi faollashdi! O'quvchilarga yubormoqchi bo'lgan xabaringizni yozib yuboring."
    )
    await c.answer()
...
async def send_ad_to_all_users(m: Message, state: FSMContext):
    if not is_admin(m.from_user.id):
        return
    await state.clear()
    status_msg = await m.answer("РІРЏС– E'lon barcha o'quvchilarga yuborilmoqda...")
    ...
    await status_msg.edit_text(
        f"СЂСџРЏВ Р С“ <b>E'lon tarqatish yakunlandi!</b>\n\n"
        f"РІСљвЂ¦ Yetkazildi: <b>{success_count} ta</b>\n"
        f"РІСњРЉ Yetkazilmadi: <b>{fail_count} ta</b>",
        reply_markup=admin_menu(),
        parse_mode="HTML",
    )
...
async def delete_subject_confirm(c: CallbackQuery):
    ...
    await c.message.edit_text(
        "РІС™В РїС‘РЏ <b>FANNI OРІР‚ВCHIRISH</b>\n\n"
        f"СЂСџвЂњС™ Fan: <b>{subject_name}</b>\n\n"
        f"СЂСџвЂњвЂ“ Mavzular: <b>{topic_count} ta</b>\n"
        f"СЂСџвЂњвЂ№ Savollar: <b>{question_count} ta</b>\n\n"
        "РІСњвЂ” <b>DIQQAT!</b>\n"
        "Bu fanni oРІР‚Вchirsangiz, unga tegishli "
        "barcha mavzular va savollar ham oРІР‚Вchiriladi.\n\n"
        "Rostdan ham oРІР‚Вchirmoqchimisiz?",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="РІСљвЂ¦ Ha, oРІР‚Вchirish",
                        callback_data=f"adm:delete_subject_confirm:{sid}",
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="РІСњРЉ Bekor qilish", callback_data=f"adm:subject:{sid}"
                    )
                ],
            ]
        ),
...
async def delete_subject(c: CallbackQuery):
    ...
    await c.message.edit_text(
        "СЂСџвЂ”вЂ <b>FAN OРІР‚ВCHIRILDI!</b>\n\n"
        f"СЂСџвЂњС™ <b>{subject_name}</b>\n\n"
        "Fan va unga tegishli mavzular hamda savollar "
        "muvaffaqiyatli oРІР‚Вchirildi.",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="СЂСџвЂњС™ Fanlar", callback_data="adm:catalog")],
                [InlineKeyboardButton(text="СЂСџРЏВ Р’В  Admin panel", callback_data="admin")],
            ]
        ),
        parse_mode="HTML",
    ...
    await c.answer("Fan oРІР‚Вchirildi.")
...
async def delete_subject_menu(c: CallbackQuery):
    ...
    await c.message.edit_text(
        "СЂСџвЂ”вЂ <b>FANNI OРІР‚ВCHIRISH</b>\n\n" "OРІР‚Вchirmoqchi boРІР‚Вlgan fanni tanlang:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons),
        parse_mode="HTML",
    )
